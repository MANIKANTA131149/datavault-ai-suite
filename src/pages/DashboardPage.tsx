...
import { Database, MessageSquare, Clock, Upload, ArrowRight, Zap, TrendingUp, TrendingDown, Minus, CheckCircle, XCircle, Shield, Activity } from "lucide-react";
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex items-start justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            {new Date().toLocaleDateString("en-US", { weekday: "long", month: "long", day: "numeric" })} · Here's your workspace at a glance
          </p>
        </div>
        <Button onClick={() => navigate("/app/query")} className="hidden sm:flex items-center gap-2">
        ...
        <Card className="p-6 bg-background-secondary border-border">
          ...
          {entries.length === 0 ? (
            <div className="text-center py-8">
              ...
              <Button variant="link" className="text-primary mt-1" onClick={() => navigate("/app/query")}>
                ...
                <div key={entry.id} className="flex items-start gap-3 py-2 border-b border-border last:border-0">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {entry.datasetName} · {PROVIDER_LABELS[entry.provider as Provider]} · {entry.totalTokens.toLocaleString()} tokens
                    </p>
                  </div>
